...
const applicationName = options.wallet ? 'TOU Wallet' : 'Mist';
...
gulp.task('bundling-interface', cb => {
  ...
  if (type === 'wallet') {
    if (options.walletSource === 'local') {
      ...
      console.log(
        `Pulling https://github.com/touristcoin/meteor-dapp-wallet/tree/${
          options.walletSource
        } "${options.walletSource}" branch...`
      );
      bundle(`&& cd ../dist_${type} \
                && git clone --depth 1 https://github.com/touristcoin/meteor-dapp-wallet.git \
...
gulp.task('build-dist', cb => {
  const appPackageJson = _.extend({}, require('../package.json'), {
    ...
    homepage: 'https://github.com/touristcoin/mist',
    build: {
      appId: `org.touristcoin.${type}`,
      asar: true,
      directories: {
