...
gulp.task('update-nodes', cb => {
  ...
  // Query latest multi-geth version
  got('https://api.github.com/repos/touristcoin/multi-geth/releases/latest', {
    json: true
  })
    .then(response => {
    ...
    .then(tagName => {
      ...
      if (cmp(latestGethVersion, localGethVersion)) {
        ...
        got(
          `https://api.github.com/repos/touristcoin/multi-geth/commits/${tagName}`,
          { json: true }
        )
          .then(response => {
          ...
          .then(hash => {
            ...
            got(
              'https://touristcoin.blob.core.windows.net/builds?restype=container&comp=list',
              { xml: true }
            )
              .then(response => {
